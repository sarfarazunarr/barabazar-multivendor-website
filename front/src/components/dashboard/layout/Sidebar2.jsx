...
const Sidebar2 = () => {
    ...
    useEffect(()=>{
        ...
        getUser();
    }, [getUser, Loggedin, login, nav, token]);
    ...
    let menuItems = [
        ...
        {
            text: 'New Product',
            link: '/vdashboard/newproduct',
        },
        {
            text: 'Products',
            link: '/vdashboard/products',
        },
        {
            text: 'Orders',
            link: '/vdashboard/orders',
        },
        {
            text: 'Logout',
    ...
    return (
        <>
            ...
            <aside id="customer-sidebar" class="fixed top-18 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 rounded-md" aria-label="Sidebar">
                ...
                <Link to="/dashboard"> <button className=' my-5 mx-5 py-3 px-5 text-white bg-gradient-to-r to-orange-500 from-yellow-500 text-xl font-semibold rounded-md hover:bg-gradient-to-bl transition-all duration-200'>Customer Profile</button></Link>
                </div>
            </aside>
        </>
    )
